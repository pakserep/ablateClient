...
static PetscErrorCode ParticleSetInitialVelocity(DM particleDm) {
    ...
    /*
    PetscScalar *vel;
    PetscInt dim, Npb, p, n;


    ierr = PetscOptionsBegin(PETSC_COMM_WORLD, "", "Box Particle Initialization Options", NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-Npb", "The initial number of particles per box dimension", NULL, Npb, &Npb, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsEnd();

    ierr = PetscPrintf(PETSC_COMM_WORLD, "***Npb=%D\n***", Npb);CHKERRQ(ierr);
    ierr = DMSwarmGetField(particleDm, ParticleVelocity, NULL, NULL, (void **) &vel);CHKERRQ(ierr);

    ierr = DMGetDimension(particleDm, &dim);CHKERRQ(ierr);
    ierr = PetscPrintf(PETSC_COMM_WORLD, "***dim=%D\n***", dim);CHKERRQ(ierr);
    ...
    ierr = DMSwarmRestoreField(particleDm, ParticleVelocity, NULL, NULL, (void **) &vel);CHKERRQ(ierr);
    */

    Vec vel;
    ierr = DMSwarmCreateGlobalVectorFromField(particleDm,ParticleVelocity, &vel);CHKERRQ(ierr);
    ierr = VecSet(vel, 0.0);CHKERRQ(ierr);
    //ierr = VecView(vel, PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
